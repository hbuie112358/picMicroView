...
import picmicroview.view.MainWindow;
...
//	private long now; //testing, remove when no longer needed
	 ...
	 * then the picmicroview.view has to be updated with the new current state of the Model (its registers,
	 ...
	 * as a group).
	 * @param mw reference to MainWindow object created in picmicroview.PicMain
	 */
	...
	/**
	 * Called by request picmicroview.controller initialize_pvt. Calls mw.updatePgmMemTable()
	 * which loads values from pic18 program memory into main window's pgmMemTable's
	 * TableModel. Called when request picmicroview.controller initializes on pgm boot, when a file
